 ...
 * @class       Tine.Expressomail.ContactGridPanel
 ...
 * @param       {Object} config
 * @constructor
 * Create a new Tine.Expressomail.ContactGridPanel
 */
Tine.Expressomail.ContactGridPanel = Ext.extend(Tine.Addressbook.ContactGridPanel, {

    hasDetailsPanel: false,
    hasFavoritesPanel: false,
    hasQuickSearchFilterToolbarPlugin: false,
    stateId: 'ExpressomailContactGrid',
    ...
    /**
     * expressomail app, needed for translations
     * @type Tine.Expressomail.Application
     */
    expressomailApp: null,
    ...
    initComponent: function() {
        ...
        this.expressomailApp = Tine.Tinebase.appMgr.get('Expressomail');
        
        Tine.Expressomail.ContactGridPanel.superclass.initComponent.call(this);
    ...
    initFilterPanel: function(config) {
        ...
        this.defaultFilters = 
                            [{"field":"query","operator":"contains","value":this.query? this.query :'@'},
                             {"field":"container_id","operator":"in","value":Tine.Addressbook.registry.get('preferences').get('autosearchAddressbook')
                            }]
        
        Tine.Expressomail.ContactGridPanel.superclass.initFilterPanel.call(this, {
     ...
     */
    getColumns: function() {
        var columns = Tine.Expressomail.ContactGridPanel.superclass.getColumns.call(this);
        
        // hide all columns except name/company/email/email_home (?)
        Ext.each(columns, function(column) {
            if (['n_fn', 'org_name', 'email'].indexOf(column.dataIndex) === -1) {
                column.hidden = true;
            }
        });
    ...
    onContactStoreLoad: function(store, records, options) {
        Ext.each(records, function(record) {
            Ext.each(['to', 'cc', 'bcc'], function(type) {
                if (this.messageRecord.data[type].indexOf(Tine.Expressomail.getEmailStringFromContact(record)) !== -1) {
    ...
    updateRecipients: function(contact, typeToSet) {
        var email = Tine.Expressomail.getEmailStringFromContact(contact),
            found = false;
        ...
        if (! found && typeToSet !== 'none') {
            Tine.log.debug('Tine.Expressomail.ContactGridPanel::updateRecipients() - adding email ' + email + ' to ' + typeToSet);
    ...
    getViewRowClass: function(record, index) {
        ...
        if (! record.hasEmail()) {
            className = 'expressomail-no-email';
        }
    ...
    initActions: function() {
        this.actions_addAsTo = new Ext.Action({
            ...
            text: this.expressomailApp.i18n._('Add as "To"'),
            disabled: true,
            iconCls: 'action_add',
        ...
        this.actions_addAsCc = new Ext.Action({
            requiredGrant: 'readGrant',
            text: this.expressomailApp.i18n._('Add as "Cc"'),
            disabled: true,
            iconCls: 'action_add',
            actionUpdater: this.updateRecipientActions,
        ...
        this.actions_addAsBcc = new Ext.Action({
            requiredGrant: 'readGrant',
            text: this.expressomailApp.i18n._('Add as "Bcc"'),
            disabled: true,
            iconCls: 'action_add',
            actionUpdater: this.updateRecipientActions,
        ...
        this.actions_setToNone = new Ext.Action({
            requiredGrant: 'readGrant',
            text: this.expressomailApp.i18n._('Remove from recipients'),
            disabled: true,
    ...
    onAddContact: function(type) {
        ...
        if (sm.isFilterSelect) {
            this['AddressLoadMask'] = new Ext.LoadMask(Ext.getBody(), {msg: this.expressomailApp.i18n._('Loading Mail Addresses')});
...
Ext.reg('expressomailcontactgrid', Tine.Expressomail.ContactGridPanel);
