 ...
 * @subpackage  Sieve
 */
class Expressomail_Sieve_Backend_Script extends Expressomail_Sieve_Backend_Abstract
{
    /**
     * the script to parse
     * 
    ...
    */
    protected function _fixEncoding($_value){
        ...
        if (!iconv('utf-8','ISO-8859-1',$_value)){ // test if utf-8
            $return = iconv('ISO-8859-1','utf-8',$_value); // convert from iso-8859-1
        } else {
            $return = $_value;
        }
    ...
    public function readScriptData()
    {
        if ($this->_scriptToParse === NULL) {
            throw new Expressomail_Exception('No script to parse has been set.');
        }
        
        $line = strtok($this->_scriptToParse, "\n");

        while ($line !== false) {
            $line = $this->_fixEncoding($line);
    ...
    protected function _parseSmartSieveRule(array $parts)
    {
        ...
        if (!empty($parts[3])) {
            ...
            $condition->setTest(Expressomail_Sieve_Rule_Condition::TEST_ADDRESS)
                ->setComperator(Expressomail_Sieve_Rule_Condition::COMPERATOR_CONTAINS)
                ->setHeader('from')
        ...
        if (!empty($parts[5])) {
            ...
            $condition->setTest(Expressomail_Sieve_Rule_Condition::TEST_HEADER)
                ->setComperator(Expressomail_Sieve_Rule_Condition::COMPERATOR_CONTAINS)
                ->setHeader('subject')
        ...
        if (!empty($parts[11])) {
            $condition = new Expressomail_Sieve_Rule_Condition();
            $comperator = empty($parts[8]) ? Expressomail_Sieve_Rule_Condition::COMPERATOR_OVER :
                    $parts[8] === 0 ? Expressomail_Sieve_Rule_Condition::COMPERATOR_UNDER :
                        Expressomail_Sieve_Rule_Condition::COMPERATOR_OVER;

            $condition->setTest(Expressomail_Sieve_Rule_Condition::TEST_SIZE)
                ->setComperator($comperator)
        ...
        switch ($this->_unescapeChars($parts[6])) {
            case ('folder'):
                ...
                break;
            case ('address'):
                $action->setType(Expressomail_Sieve_Rule_Action::REDIRECT)
                    ->setArgument($this->_unescapeChars($parts[7]));
                break;
            case ('reject'):
                $action->setType(Expressomail_Sieve_Rule_Action::REJECT)
                    ->setArgument($this->_unescapeChars($parts[7]));
                break;
            case ('discard'):
    ...
    protected function _getPseudoScript()
    {
        ...
        if ($this->_vacation instanceof Expressomail_Sieve_Vacation) {
