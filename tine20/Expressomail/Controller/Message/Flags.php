 ...
 * @subpackage  Controller
 */
class Expressomail_Controller_Message_Flags extends Expressomail_Controller_Message
{
    /**
     * imap flags to constants translation
     * @var array
     */
    protected static $_allowedFlags = array('Passed'    => Zend_Mail_Storage::FLAG_PASSED,
                                            'Read'      => Zend_Mail_Storage::FLAG_READ,
     ...
     *
     * @var Expressomail_Controller_Message_Flags
     */
    ...
    protected function _addOrClearFlags($_messages, $_flags, $_mode = 'add')
    {
        ...
        if($_mode == 'add' && $flags[0] == '\Deleted'){
                    $folders = $affectedFolders->toArray();
                    foreach( $folders as $folder){
                        $imapBackend->expunge($folder['globalname']);
                    }
        }
    ...
    public function getSupportedFlags($_translated = TRUE)
    {
        if ($_translated) {
            ...
            foreach (self::$_allowedFlags as $flag) {
                $customFlag = ($flag[0] == '\\') ? substr($flag, 1) : $flag;
                $result[] = array('id'        => $flag,      'name'      => $translate->_($customFlag));
            }
...
//    }
     ...
     */
    public function setSeenFlag(Expressomail_Model_Message $_message)
    {
