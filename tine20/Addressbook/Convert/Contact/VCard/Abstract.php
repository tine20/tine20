    ...
    public function toTine20Model($blob, Tinebase_Record_Abstract $_record = null, $options = array())
    {
        ...
        foreach ($vcard->children() as $property) {
            switch ($property->name) {
                ...
                case 'PHOTO':
                    if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) 
                        Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Photo: ' . $property['Endocing'] . ":" . $property['TYPE'] );
			...
			case 'png' : {}
                        case 'JPEG' : {
                            if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) 
                                Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Photo: passing on jpeg image as is (' . strlen($property->getValue()) .')' );
...
/*
			    $binimage = imagecreatefromstring($property->getValue());
                            if ( $binimage === false ) {
                                $jpegphoto = null;
                            if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) 
                                Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Photo: imageformat unkown or not supported ' );
                                break;
                            }*/
                            ...
                            if ( in_array($info['mime'], Tinebase_ImageHelper::getSupportedImageMimeTypes()) ) {
                                if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) 
                                    Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Photo: not recoded ' );
                            	$jpegphoto = $property->getValue();
                            	break;
                            }
                            ...
                            if ( !in_array($info['mime'], Tinebase_ImageHelper::getSupportedImageMimeTypes()) ) {
                                if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG))
                                    Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Photo: failed to recode to jpeg (' .strlen($jpegphoto) .')' );
                                $jpegphoto = null;
                                break;
                            }
                            if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) 
