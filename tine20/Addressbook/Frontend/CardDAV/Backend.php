 ...
 * @package     Calendar
 */
class Addressbook_Frontend_CardDAV_Backend extends Sabre_CardDAV_Backend_Abstract
{
    /**
    ...
    function getAddressBooksForUser($principalUri)
    {
        ...
        if (count($principalParts) == 2) {
            ...
            $containers = Tinebase_Container::getInstance()->getPersonalContainer(Tinebase_Core::getUser(), 'Addressbook', $owner, Tinebase_Model_Grants::GRANT_READ);
        } else {
            throw new Sabre_DAV_Exception_PreconditionFailed('unsupported pricipalUri');
        }
        //if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' $conatiners: ' . print_r($containers->toArray(), true));
        $addressbooks = array();
        ...
        foreach($containers as $container) {
            ...
            $addressbooks[] = array(
                ...
                '{' . Sabre_CardDAV_Plugin::NS_CARDDAV . '}addressbook-description' => $container->name,
    ...
    {
        throw new Sabre_DAV_Exception_MethodNotAllowed('createAddressBook');
    }
    ...
    {
        throw new Sabre_DAV_Exception_MethodNotAllowed('updateAddressBook');
    }
    ...
    {
        throw new Sabre_DAV_Exception_MethodNotAllowed('deleteAddressBook');
    }
    ...
    */
    ...
    {
        if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' adressbook ID: ' . $addressbookId);
        ...
        //if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' contacts ' . print_r($contacts->toArray(), true));
        
        foreach ($contacts as $contact) {
            $cards[] = array(
                        'id' => $contact->getId(),
                        'carddata' => $this->_convertToVobject($contact)->serialize(),
                        'uri' => $contact->getId(),
                        'lastmodified' => $contact->last_modified_time instanceof Tinebase_DateTime ? $contact->last_modified_time->toString() : $contact->creation_time->toString() 
            );
        }
     ...
     */
    protected function _convertToVobject(Addressbook_Model_Contact $_contact)
    {
    ...
    /**
     * convert Sabre_VObject_Element to Addressbook_Model_Contact
     */
    protected function _convertToAddressbookModelContact(Sabre_VObject_Component $_vcard, Addressbook_Model_Contact $_contact = null)
    {
        ...
        foreach($_vcard->children() as $property) {
    ...
    {
        if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' cardUri ' . $cardUri);
        
        $contact = Addressbook_Controller_Contact::getInstance()->get($cardUri);
    ...
    {
        if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' cardUri ' . $cardUri);
        if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' cardData ' . $cardData);
    ...
    public function updateCard($addressBookId, $cardUri, $cardData) 
    {
        ...
        $vcard = Sabre_VObject_Reader::read($cardData);
        
        $contact = Addressbook_Controller_Contact::getInstance()->get($cardUri);
        
        $contact = $this->_convertToAddressbookModelContact($vcard, $contact);
