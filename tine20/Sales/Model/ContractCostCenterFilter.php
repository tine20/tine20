...
/**
 * Contract-Contract-CostCenter filter Class
*
* @package     Sales
* @subpackage  Model
*/
class Sales_Model_ContractCostCenterFilter extends Tinebase_Model_Filter_Relation
{
    /**
        ...
        if(! empty($this->_value[0]['value'])) {
            ...
            foreach($ret['value'] as &$filter) {
                if($filter['field'] == ':id' && $filter['operator'] == 'equals' && is_string($filter['value']) && strlen($filter['value']) == 40) {
                    $filterValue = Sales_Controller_CostCenter::getInstance()->get($filter['value']);
            ...
            if (! $found) {
                $filterValueRecord = Sales_Controller_CostCenter::getInstance()->get($this->_value[0]['value']);
