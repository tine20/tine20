...
/**
 * Invoice-Contract filter Class
 *
 * @package     Sales
 * @subpackage  Model
 */
class Sales_Model_InvoiceContractFilter extends Tinebase_Model_Filter_Relation
{
    /**
    ...
    public function toArray($_valueToJson = false)
    {
        ...
        foreach($ret['value'] as &$filter) {
            if ($filter['field'] == ':id' && $filter['operator'] == 'equals' && is_string($filter['value']) && strlen($filter['value']) == 40) {
                $fr = Sales_Controller_Contract::getInstance()->get($filter['value']);
                $fr->relations = null;
