...
class ActiveSync_Command_Sync extends ActiveSync_Command_Wbxml 
{
    ...
    public function handle()
    {
        ...
        $sync = $this->_outputDom->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Sync'));
        ...
        foreach ($xml->Collections->Collection as $xmlCollection) {
            ...
            if($clientSyncKey === 0) {
                ...
                $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Class', $class));
                $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'SyncKey', $newSyncKey));
                $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'CollectionId', $collectionId));
                $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Status', self::STATUS_SUCCESS));
    
                if(!empty($this->_added)) {
                    $response = $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Responses'));
                    foreach($this->_added as $serverId => $clientId) {
                        $add = $response->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Add'));
                ...
                $collection = $collections->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Collection'));
                $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Class', $class));
                $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'SyncKey', 1));
                $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'CollectionId', $collectionId));
                ...
                #$sync = $this->_outputDom->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Sync'));
                #$sync->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Status', self::STATUS_INVALID_SYNC_KEY));
    ...
    protected function _handleSync0($_collections, $_class, $_collectionId)
    {
        ...
        $collection = $_collections->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Collection'));
        $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Class', $_class));
        $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'SyncKey', 1));
        $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'CollectionId', $_collectionId));
        $collection->appendChild($this->_outputDom->createElementNS('uri:AirSync', 'Status', self::STATUS_SUCCESS));
        
        Tinebase_Core::getLogger()->info(__METHOD__ . '::' . __LINE__ . " new synckey is 1");
        
        // store new synckey
        ActiveSync_Controller::getInstance()->updateSyncKey($this->_device, 1, $_class . '-' . $_collectionId, $this->_syncTimeStamp);        
    }
    ...
    protected function _handleInitialSync($_xml, $_collections, $_collectionId)
    {
        ...
        if(isset($_xml->Collections->Collection->Commands->Add)) {
            $adds = $_xml->Collections->Collection->Commands->Add;
    ...
    protected function _handleIncrementalSync($_xml, $_collections, $_collectionId)
    {
        ...
        if(isset($_xml->Collections->Collection->Commands->Change)) {
            $changes = $_xml->Collections->Collection->Commands->Change;
        ...
        if(isset($_xml->Collections->Collection->Commands->Delete)) {
            $deletes = $_xml->Collections->Collection->Commands->Delete;
