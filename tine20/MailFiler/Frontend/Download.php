<?php
/**
 * MailFiler public download frontend
 *
 * This class handles all public download requests for the MailFiler application
 * 
 ...
 */
class MailFiler_Frontend_Download extends Tinebase_Frontend_Http_Abstract
{
    /**
     * display download
     * 
    ...
    public function displayNode($path)
    {
        try {
            ...
            $node = MailFiler_Controller_DownloadLink::getInstance()->getNode($download, $splittedPath);
    ...
    public function downloadNode($path)
    {
        try {
            ...
            if ($node->type === Tinebase_Model_Tree_Node::TYPE_FILE) {
                ...
                MailFiler_Controller_DownloadLink::getInstance()->increaseAccessCount($download);
            ...
            $view = new Zend_View();
            $view->setScriptPath('MailFiler/views');
     ...
     * 
     * @param  string $id
     * @return MailFiler_Model_DownloadLink
     */
    protected function _getDownloadLink($id)
    {
        $download = MailFiler_Controller_DownloadLink::getInstance()->get($id);
     ...
     * 
     * @param MailFiler_Model_DownloadLink $download
     * @param Tinebase_Model_Tree_Node       $node
     * @param array                          $path
     */
    protected function _listDirectory(MailFiler_Model_DownloadLink $download, Tinebase_Model_Tree_Node $node, $path)
    {
        $this->_getView($path, $node);
        $view->files = MailFiler_Controller_DownloadLink::getInstance()->getFileList($download, $path, $node);
     ...
     * @param array                          $path
     */
    protected function _displayFile(MailFiler_Model_DownloadLink $download, Tinebase_Model_Tree_Node $node, $path)
    {
     ...
     * @param MailFiler_Model_DownloadLink $download
     */
    protected function _setDownloadLinkOwnerAsUser(MailFiler_Model_DownloadLink $download)
    {
