...
class Tinebase_Acl_Roles extends Tinebase_Controller_Record_Abstract
{
    ...
    protected $_allowDeleteDefaulRoles = false;
    ...
    /**
     * @param Tinebase_Model_Role $updatedRecord
     * @param Tinebase_Model_Role $record
     * @param Tinebase_Model_Role $currentRecord
     */
    protected function _inspectAfterUpdate($updatedRecord, $record, $currentRecord)
    {
        parent::_inspectAfterUpdate($updatedRecord, $record, $currentRecord); // TODO: Change the autogenerated stub

        if ($updatedRecord->name !== $currentRecord->name) {
            if (Tinebase_Config::getInstance()->{Tinebase_Config::DEFAULT_ADMIN_ROLE_NAME} === $currentRecord->name) {
                Tinebase_Config::getInstance()->{Tinebase_Config::DEFAULT_ADMIN_ROLE_NAME} = $updatedRecord->name;
            } elseif (Tinebase_Config::getInstance()->{Tinebase_Config::DEFAULT_USER_ROLE_NAME} === $currentRecord->name) {
                Tinebase_Config::getInstance()->{Tinebase_Config::DEFAULT_USER_ROLE_NAME} = $updatedRecord->name;
            }
        }
    }
    ...
    protected function _inspectDelete(array $_ids)
    {
        ...
        if (!$this->_allowDeleteDefaulRoles) {
            foreach ($this->_backend->getMultiple($_ids) as $role) {
                if (Tinebase_Config::getInstance()->{Tinebase_Config::DEFAULT_ADMIN_ROLE_NAME} === $role->name) {
                    Tinebase_Core::getLogger()->err(__METHOD__ . '::' . __LINE__ . ' default admin role can\'t be deleted');
                    unset($_ids[array_search($role->getId(), $_ids)]);
                } elseif (Tinebase_Config::getInstance()->{Tinebase_Config::DEFAULT_USER_ROLE_NAME} === $role->name) {
                    Tinebase_Core::getLogger()->err(__METHOD__ . '::' . __LINE__ . ' default user role can\'t be deleted');
                    unset($_ids[array_search($role->getId(), $_ids)]);
                }
            }
        }
    ...
    {
        $this->_allowDeleteDefaulRoles = true;
