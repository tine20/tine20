...
class Setup_Controller
{
    ...
    protected function _enforceCollation()
    {
        ...
        $tables = $db->query('SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME LIKE "' .
            SQL_TABLE_PREFIX . '%" AND TABLE_COLLATION <> "' . $collation . '"')->fetchAll(Zend_DB::FETCH_COLUMN, 0);
        ...
        $columns = $db->query('SELECT TABLE_NAME, COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME LIKE "' .
            SQL_TABLE_PREFIX . '%" AND COLLATION_NAME <> "' . $collation . '" AND COLLATION_NAME IS NOT NULL AND (' .
            'TABLE_NAME <> "' . SQL_TABLE_PREFIX . 'tree_nodes" OR COLUMN_NAME <> "name")')
            ->fetchAll();
