...
describe('Contacts', () => {
    describe('Test MainPage', () => {
        ...
        test('show map', async () => {
            ...
            if (!(await popupWindow.$x('//li[contains(@class, " x-item-disabled") and contains(., "Karte")]'))) {
                await selectTab(popupWindow, 'Karte');
                await popupWindow.waitForTimeout(10000); // wait to load map
                await popupWindow.screenshot({path: 'screenshots/Adressbuch/12_adressbuch_kontakt_karte.png'});
            } else {
        ...
        test('attachments', async () => {
            ...
            await popupWindow.screenshot({path: 'screenshots/StandardBedienhinweise/22_standardbedienhinweise_hr_mitarbeiter_anhang.png'});
        });

        test('relations', async () => {
            await selectTab(popupWindow, 'Verknüpfungen.*');
            await popupWindow.waitForSelector('.x-panel.x-wdgt-pickergrid.x-grid-panel');
            await popupWindow.screenshot({path: 'screenshots/StandardBedienhinweise/23_standardbedienhinweise_hr_mitarbeiter_verknuepfungen.png'});
            let arrowtrigger = await popupWindow.$$('.x-form-arrow-trigger');
            await arrowtrigger[9].click();
            await popupWindow.screenshot({path: 'screenshots/Adressbuch/13_adressbuch_kontakt_bearbeiten_verknuepfung_links.png'});
            await popupWindow.screenshot({path: 'screenshots/StandardBedienhinweise/24_standardbedienhinweise_hr_mitarbeiter_verknuepfungen_hinzu.png'});
            await arrowtrigger[10].click();
            await popupWindow.screenshot({path: 'screenshots/Adressbuch/14_adressbuch_kontakt_bearbeiten_verknuepfung_rechts.png'});
        });

        test('history', async () => {
            await selectTab(popupWindow, 'Historie.*');
            await popupWindow.screenshot({path: 'screenshots/StandardBedienhinweise/21_standardbedienhinweise_hr_mitarbeiter_historie.png'});
