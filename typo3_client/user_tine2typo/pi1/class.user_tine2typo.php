...
//	var $languageFile = ;
	...
	{
		$xml = simplexml_load_file(PATH_site . 'typo3conf/ext/user_tine2typo/locallang_db.xml');
		$labels = ($xml->data->languageKey->label);
		...
		//reset($contact);		
		$subpart_template = $this->cObj->getSubpart($template,'###TDTEMPLATE###');
		foreach ($cols as  $col)
		{
			if (!empty($col))
			{
				...
				$trow_ .= $this->cObj->substituteMarkerArray($subpart_template, $markerArray);
			}
		}
		$subpart_template = $this->cObj->getSubpart($template,'###TRTEMPLATE###');
		$TROW .= $this->cObj->substituteMarkerArray($subpart_template, array('###TRBODY###' => $trow_));
			...
			//$this->cObj->substituteMarkerArray($subpart_template, $markerArray);	
			//#################### Initialisierung #####################
