...
if [ "${REUSE}" = "false" ]; then
    echo "building image ..."
    ${CI_BUILDS_DIR}/tine20/tine20/ci/scripts/build_image.sh $TARGET
    exit 0
fi
...
if curl -s -f --user $REGISTRY_USER:$REGISTRY_PASSWORD -H "accept: application/vnd.docker.distribution.manifest.v2+json" "https://${REGISTRY}/v2/${TARGET}/manifests/${CI_COMMIT_REF_NAME_ESCAPED}-${PHP_VERSION}" > /dev/null; then
    ...
    ${CI_BUILDS_DIR}/tine20/tine20/ci/scripts/rename_remote_image.sh $REGISTRY_USER $REGISTRY_PASSWORD $REGISTRY ${TARGET} ${CI_COMMIT_REF_NAME_ESCAPED}-${PHP_VERSION} ${TARGET}-commit ${IMAGE_TAG}
    exit 0
fi
...
if curl -s -f --user $REGISTRY_USER:$REGISTRY_PASSWORD -H "accept: application/vnd.docker.distribution.manifest.v2+json" "https://${REGISTRY}/v2/${TARGET}/manifests/${MAJOR_COMMIT_REF_NAME_ESCAPED}-${PHP_VERSION}" > /dev/null; then
    ...
    ${CI_BUILDS_DIR}/tine20/tine20/ci/scripts/rename_remote_image.sh $REGISTRY_USER $REGISTRY_PASSWORD $REGISTRY ${TARGET} ${MAJOR_COMMIT_REF_NAME_ESCAPED}-${PHP_VERSION} ${TARGET}-commit ${IMAGE_TAG}
    exit 0
fi
